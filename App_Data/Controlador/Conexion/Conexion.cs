...
public class Conexion
{
    ...
    public Conexion()
    {
        this.conn = "DB_GestionDeAlmacenConnectionString";
    }
    ...
    #endregion
    /// <summary>
    /// Esta funcion te permite realiar la consulta de un procedimiento regresando los valores en las 
    /// propiedades del objeto que se espesifica 
    /// </summary>
    /// <typeparam name="Object"></typeparam>
    /// <param name="atributos"></param>
    /// <param name="obj"></param>
    /// <returns></returns>
    public string ejecutarProcRecuperarObjeto<Object>(string[] atributos, Object obj)
    {
        string msj;
        try
        {
            //ejecuta el procedimiento 
            leer = comandoSP.ExecuteReader();
            //valida si tiene informacion la consulta
            if (leer.HasRows)
            {
                ...
                while (leer.Read())
                {
                    var Data = obj;
                    PropertyInfo[] Properties = Data.GetType().GetProperties();
                    //recorre el objeto para agregar los valores 
                    foreach (var p in Properties)
                    {
                        if (Array.IndexOf(atributos, p.Name) >= 0)
                        {
    ...
    public string ejecutaProcRecuperaObjetoLista<T>(string[] atributos, List<T> Lista) where T : new()
    {
        try
        {
            //ejecuta el procedimiento 
    ...
    public string[] ejecutarConsultaRegistroSimpleProc()
    {
        string[] res = new string[2];
        string resCon;
        string msj;
    ...
    /// <summary>
    /// funcion para obtener multiples registros en eun dataset a traves de un SP
    /// </summary>
    /// <param name="nombre"></param>
    /// <returns></returns>
    public DataSet ejecutarProcRegistroMultiplesDataSet(string nombre)
    {
        DataSet res = new DataSet();
        try
        {
